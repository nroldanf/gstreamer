...
# Create the elements
#   1st param = Type of element
#   2nd param = Name of the instance
#   videotestsrc creates a video pattern
#   autovideosink displays video on a window. instantiates best video sink (platform independent)
source = Gst.ElementFactory.make("videotestsrc", "source")
# loopback alpha blending effector with rotating and scaling.
...
# Build the pipeline
pipeline.add(source, filter, convert, sink)
source.link(filter)
filter.link(convert)
convert.link(sink)
# if not (source.link(filter) or filter.link(convert) or convert.link(sink)):
